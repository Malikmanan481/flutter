...
class HomeCardListWidget extends StatelessWidget {
  ...
  HomeCardListWidget({this.index, this.context, this.homeItemList});
                  ...
                  Container(
                    ...
                    child: Row(
                      ...
                      children: <Widget>[
                                ...
                                children: [
                                  Container(
                                      ...
                                      child: CachedNetworkImage(
                                        imageUrl: Globals.baseUrl +
                                            homeItemList!.brandImage,
                                        width: 25,
                                        height: 25,
                        ...
                        Image.asset(
                          'images/${Helper().getMarkerName(homeItemList!.status, Globals.prefs!.getString('${homeItemList!.imei}_icons')!)}',
                          width: 35,
                          height: 35,
                        ),
                        SizedBox(
                            ...
                            child: Row(
                              ...
                              children: [
                                homeItemList!.status == "s" &&
                                        homeItemList!.status != "m"
                                    ? Row(
                                        mainAxisAlignment:
...
class CardButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Flexible(
      flex: flexSize!,
      child: InkWell(
        onTap: () => Navigator.of(context).push(onClickMethod!),
        child: SizedBox(
          height: 50,
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
...
// ======================================================
// TRACCAR REST API BACKEND CONNECTOR FOR LIST WIDGET
// ======================================================

class TraccarListWidgetBackend {
  /// Fetches devices & positions from Traccar endpoints (/api/devices, /api/positions)
  /// Maps raw Traccar API response directly into the HomeItem object used by HomeCardListWidget.
  static Future<List<HomeItem>> fetchTraccarDevicesList({
    ...
    try {
      ...
      if (devicesResponse.statusCode == 200 &&
          positionsResponse.statusCode == 200) {
        final List<dynamic> devices = jsonDecode(devicesResponse.body);
        final List<dynamic> positions = jsonDecode(positionsResponse.body);

        // Map positions by deviceId for quick lookup
        final Map<int, Map<String, dynamic>> positionMap = {};
        for (var pos in positions) {
          positionMap[pos['deviceId'] as int] = Map<String, dynamic>.from(pos);
        }
        ...
        for (var device in devices) {
          ...
          final pos = positionMap[deviceId];

          if (pos != null) {
            final Map<String, dynamic> attributes = pos['attributes'] ?? {};
            final double speedKnots = (pos['speed'] ?? 0.0).toDouble();
            final int speedKmH = (speedKnots * 1.852).round();

            // Traccar status mapping logic -> 'm', 's', 'i', 'off', 'false'
            String appStatus = 'off';
            final String serverStatus = device['status'] ?? 'offline';
            ...
            homeItems.add(
              HomeItem(
                ...
                brandImage: device['category'] != null
                    ? '/images/${device['category']}.png'
                    : '',
                expired: device['disabled'] ?? false,
                expiredText: device['expirationTime'] ?? '',
