...
class PlaybackActivity extends StatefulWidget {
  final String? name;
  final String? imei;
  final String? lat;
  final String? lng;
  ...
  @override
  _PlaybackActivityState createState() =>
      _PlaybackActivityState(name!, imei!, lat!, lng!, angle!, status!, speed!);
}

class _PlaybackActivityState extends State<PlaybackActivity> {
  final String? name;
  final String? imei;
  final String? lat;
  ...
  String? dtf, dtt;
  String buttonText = 'playback'.tr;

  _PlaybackActivityState(this.name, this.imei, this.lat, this.lng, this.angle,
      this.status, this.speed);

  /// Traccar Device ID fetch karne ke liye backend API lookup
  Future<void> _fetchTraccarDeviceId() async {
    try {
  ...
  differentTimes() {
    ...
    var formatterForHour = new DateFormat('yyyy-MM-dd HH:mm:ss');
  ...
  void setMarker() async {
    BitmapDescriptor bitmapDescriptor = await Helper().getMarker(
        ...
        speed!,
        'images/'
            '${Helper().getMarkerName(status!, Globals.prefs!.getString('${imei}_icons')!)}');
  ...
  /// Traccar API Compatible ISO Timestamp Formatter
    ...
    return Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            '${name!}',
            style: TextStyle(
                color: Globals.prefs!.getBool('mapHybrid')!
                    ? Color(0xFFF6F6F6)
                    : Globals.appColor,
        ...
        body: Stack(
          children: [
            GoogleMap(
                ...
                mapType: Globals.prefs!.getBool('mapHybrid')!
                    ? MapType.hybrid
                    : MapType.normal,
                      ...
                      children: <Widget>[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                              border:
                                  Border.all(color: Globals.appColor, width: 2),
                              borderRadius:
                          ...
                          child: Center(
                            child: CustomProgressButton(
                              onPressed: () async {
                                ...
                                String traccarDtf = formatToTraccarIso(dtf!);
                                String traccarDtt = formatToTraccarIso(dtt!);
                                    ...
                                    CupertinoPageRoute(
                                        builder: (context) =>
                                            BlocProvider<WindowBloc>(
                                                create: (context) =>
                                                    WindowBloc(),
                                                child: HistoryMapActivity(
                                                    name: name,
                                                    imei: traccarDeviceId != null
                                                        ? '$traccarDeviceId'
                                                        : imei,
                                                    from: 'trip',
                                                    dtf: traccarDtf,
