...
class SphericalUtil {
  ...
  static double angleLerp(double from, double to, double t) {
    double radFrom = MathUtil.toRadians(from).toDouble();
    double radTo = MathUtil.toRadians(to).toDouble();
    double diff = radFrom + angleShortestDistance(radFrom, radTo) * t;
    return MathUtil.toDegrees(diff).toDouble();
  }
  ...
  static double bearingBetweenLocations(LatLng latLngFrom, LatLng latLngTo) {
    ...
    brng = MathUtil.toDegrees(brng).toDouble();
    brng = (brng + 360) % 360;
  ...
  // ==========================================
  // TRACCAR API POSITIONS & ROUTE HELPERS
  // ==========================================

  /// Parses single Traccar `/api/positions` JSON Map to Google Maps [LatLng]
  static LatLng? positionToLatLng(Map<String, dynamic> positionJson) {
    if (positionJson.containsKey('latitude') &&
        positionJson.containsKey('longitude')) {
      double? lat = double.tryParse(positionJson['latitude'].toString());
      double? lng = double.tryParse(positionJson['longitude'].toString());
  ...
  /// Parses Traccar `/api/positions` or `/api/reports/route` JSON Array to [List<LatLng>]
  static List<LatLng> parseTraccarPositions(List<dynamic> positionsArray) {
    List<LatLng> routePoints = [];
    for (var pos in positionsArray) {
      if (pos is Map<String, dynamic>) {
        LatLng? point = positionToLatLng(pos);
        if (point != null) {
  ...
  /// Generates [LatLngBounds] to fit full Traccar route history on Google Maps
  static LatLngBounds? computeTraccarBounds(List<LatLng> points) {
  ...
  /// Calculates total accumulated route distance in meters from Traccar position list
  static double computeTotalRouteDistance(List<LatLng> points) {
    double totalDistance = 0.0;
    for (int i = 0; i < points.length - 1; i++) {
      totalDistance += computeDistanceBetween(points[i], points[i + 1]);
    }
