...
class LatLngInterpolationStream {
  ...
  // ==========================================
  // TRACCAR API POSITIONS & WEBSOCKET INTEGRATION
  // ==========================================

  /// Feed raw Traccar position JSON payload from `/api/positions` or `/api/socket`
  void addTraccarPosition(Map<String, dynamic> positionJson) {
    if (positionJson.containsKey('latitude') && positionJson.containsKey('longitude')) {
      ...
      if (lat != null && lng != null) {
        addLatLng(LatLng(lat, lng));
      }
    }
  }

  /// Push route history positions from `/api/reports/route` into animation sequence
  ...
  Stream<LatLngDelta> getLatLngInterpolation() async* {
    ...
    await for (LatLngDelta deltaPosition in _latLngRotationStream!.stream) {
      double angle = SphericalUtil.angleShortestDistance(
          MathUtil.toRadians(lastBearing).toDouble(),
          MathUtil.toRadians(deltaPosition.rotation!).toDouble());
      double angleDelta = MathUtil.toDegrees(angle).toDouble();
